...
My philosophy is therefore **mesomorphic** and **hybrid**, much like our modern screens (i.e., liquid crystal displays): primary languages and code that are **solid**, precise, structured, concise, and verifiable (by machines); but runtime environments, code, and secondary languages that are **fluid**, flexible, and adaptable, taking into account a company’s needs, history, and legacy.
...
Because it’s a language directly **rooted** in the most **advanced** research (i.e., Haskell), with very high conceptual density, particularly powerful given the modern preference for **type safety**, and also very pragmatic, as it’s **open** to **legacy** (FFI). It is therefore a language capable of intelligently **integrating** with existing systems, capable of working very closely with completely different languages. When you code in PureScript, you’re actually also coding (if desired) in Go, C++, Scheme, PHP, Python, Lua, JavaScript/TypeScript, Erlang, and so on. And you don’t do this in a scattered way; you do it within the **same codebase**. PureScript is a **central hub** that leverages each of these languages/ecosystems/runtimes, along with their respective strengths. It is therefore not an ordinary language, in the usual sense of the term. It is also a proxy language with very high added value (abstractions, design, underlying theory, conventions, etc.). 
