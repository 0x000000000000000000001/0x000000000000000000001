...
That said, I haven’t abandoned my pragmatism, and I’m very aware of the technologies still in use in industry today. My philosophy is therefore mesomorphic and hybrid, much like our modern screens (i.e., liquid crystal displays): primary languages and code that are solid, precise, structured, concise, and verifiable (by the machine); but runtimes, code, and secondary languages that are fluid, flexible, and adaptable, taking into account the contingencies of the moment within a company. On the technical team side: an accessible, structured, familiar environment that stays true to the core concept (centered on the business). On the code execution side (clients, constraints, etc.): a tailor-made environment adapted to the challenge and the contingencies of the moment and location (e.g., resilience, speed, etc.).
